...
class GumbelSoftmax(nn.Module):
    ...
    def __init__(self, f_dim, c_dim):
        ...
        self.logits = nn.Linear(f_dim, c_dim, dtype=torch.float64)
        self.f_dim = f_dim
        self.c_dim = c_dim
...
class Gaussian(nn.Module):
    def __init__(self, in_dim, z_dim):
        ...
        self.mu = nn.Linear(in_dim, z_dim, dtype=torch.float64)
        self.var = nn.Linear(in_dim, z_dim, dtype=torch.float64)
...
class InferenceNet(nn.Module):
    def __init__(self, x_dim, z_dim, y_dim):
        ...
        self.inference_qzyx = torch.nn.ModuleList([
            nn.Linear(x_dim + y_dim, 512, dtype=torch.float64),
            nn.ReLU(),
            Gaussian(512, z_dim)
        ])

        self.y_mu = nn.Linear(y_dim, z_dim, dtype=torch.float64)
        self.y_var = nn.Linear(y_dim, z_dim, dtype=torch.float64)
