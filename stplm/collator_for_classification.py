...
class PrecollatorForGeneAndCellClassification(SpecialTokensMixin):
    ...
    def pad(
        ...
        data = batch_outputs['neighborhood']
        filtered_lengths = [len(inner) for outer in data if outer for inner in outer if inner]
        ...
        padded_data = [
            [
                inner + [0] * (max_length - len(inner)) if inner else [0] * max_length
                for inner in outer
            ]if outer else []
            for outer in data
        ]
        ...
        '''
        tensor_data = []
        for outer in padded_data:
            if outer:
                tensor_outer = [torch.tensor(inner, dtype=torch.long) for inner in outer]
                tensor_data.append(tensor_outer)
            else:
                tensor_data.append([])
        '''
                # Find the maximum number of outer lists
        max_outer = len(padded_data)

        # Find the maximum number of inner lists and their lengths
        max_inner = max(len(outer) for outer in padded_data)
        filtered_lengths = [len(inner) for outer in padded_data if outer for inner in outer if inner]
        if not filtered_lengths:
            max_length = 1
        ...
        padded_tensor_data = [
            [
                inner + [0] * (max_length - len(inner)) if inner else [0] * max_length
                for inner in outer
            ] + [[0] * max_length] * (max_inner - len(outer))  # Pad inner lists
            for outer in padded_data
        ] + [[[0] * max_length] * max_inner] * (max_outer - len(padded_data))  # Pad outer lists

        
        tensor_data = torch.tensor(padded_tensor_data, dtype=torch.long)
        #tensor_data = tensor_data[:,0,:]
        batch_outputs['neighborhood'] = tensor_data
...
class DataCollatorForCellClassification(DataCollatorForGeneClassification):
    def __init__(self, add_cls=True, *args, **kwargs):
        ...
        self.tokenizer = PrecollatorForGeneAndCellClassification()
        ...
        if self.add_cls:
            for i in range(len(features)):
                features[i]['input_ids'] = (
                    [int(self.tokenizer.cls_token_id)] + features[i]['input_ids']
                )[:2048]
